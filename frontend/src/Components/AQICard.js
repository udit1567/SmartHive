...
const AQICard = () => {
  ...
  const fetchMoistureData = () => {
    if (token && id) {
      axios({
        method: "GET",
        url: `http://127.0.0.1:5000/get_d4/${id}`,
        headers: {
          Authorization: "Bearer " + token,
        },
      })
        .then((response) => {
          const data = response.data.D4; // Extracting 'D3' from API response
  ...
  const getMoistureColor = () => {
    ...
    if (moisture < 150) return "linear-gradient(to right,rgb(133, 255, 77),rgb(45, 14, 14))"; // Dry (Red)
    if (moisture > 300) return "linear-gradient(to right,rgb(5, 5, 5),rgb(255, 112, 112))"; // Well-Watered (Green)
